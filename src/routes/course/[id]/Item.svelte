...
<a href="/course/{courseId}/item/{item.id}">
    <div class="item">
        ...
        <div class="cell">
            <div class="title black" class:disabled={item.locked}>{item.name}</div>
            <div class="description disabled">{item.description}</div>
        </div>
        <div class="cell cell-big">
            ...
            {/if}
            {#if !item.repo_created}
                <div class="row red">
                    <Icon icon="mdi:git" inline style="font-size: 24px;" />
                    <div>Missing repository <i>{item.repository_name}</i></div>
                </div>
            {/if}
        </div>
        <div class="cell cell-sm black">
            <div class="row">
                ...
                <div>Grade: {item.grade} / 20</div>
                <div class="description ml-1">({item.factor_percentage} %)</div>
            </div>
        </div>
        <div class="cell blue">
            <div class="row text-right">
                ...
                <div class="column ml-1 ">
                    <div>From: {jsDateToHumanDate(item.start)}</div>
					<div>To: <span class="bold">{jsDateToHumanTime(item.end)}</span></div>
                </div>
            </div>
        </div>
    </div>
