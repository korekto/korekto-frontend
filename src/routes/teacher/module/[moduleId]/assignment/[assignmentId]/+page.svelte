<script lang="ts">
	...
	import { getTeacherAssignment, updateTeacherAssignment } from '$lib/api';
	import type { TeacherAssignmentForm } from '$lib/types';
	import Assignment from './../Assignment.svelte';

	export let data;

	let assignmentPromise = getTeacherAssignment(data.moduleId, data.assignmentId);

	const onSave = async (m: TeacherAssignmentForm) => {
		await updateTeacherAssignment(data.moduleId, data.assignmentId, m);
